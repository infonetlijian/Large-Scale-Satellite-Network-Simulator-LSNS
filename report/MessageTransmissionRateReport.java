...
import static routing.RelayRouterforInternetAccess.accumulativeHandoverDelay;
import static routing.RelayRouterforInternetAccess.handoverCount;

/**
 * Project Name:Large-scale Satellite Networks Simulator (LSNS)
 * File VMessageTransmissionRateReport.java
 * Package Name:report
 * Description: Report for generating different kind of total statistics about message
 *  relaying performance. Messages that were created during the warm up period
 *  are ignored. <P><strong>Note:</strong> if some statistics could not be created (e.g.
...
public class MessageTransmissionRateReport extends Report implements MessageListener {
    ...
    private double nrofRelayedBytes;
    private double simTime;

    /**
     * Constructor.
     */
    public MessageTransmissionRateReport() {
        init();
    }
    ...
    protected void init() {
        ...
        this.nrofRelayedBytes = 0;

        Settings s = new Settings(DTNSim.SCENARIO);
        this.simTime = s.getDouble(DTNSim.ENDTIME);
    }
    ...
    public void messageTransferred(Message m, DTNHost from, DTNHost to,
        ...
        if (finalTarget) {
            ...
            this.nrofRelayedBytes = this.nrofRelayedBytes + m.getSize();
    ...
    public void done() {
        ...
        int count = 0;
        for (DTNHost user : handoverCount.keySet()){
            count = handoverCount.get(user);
        }
        String statsText = "created: " + this.nrofCreated +
                ...
                "\ntotal_delivered_bits: " + this.nrofRelayedBytes +
                "\nrate_avg(bps): " + this.nrofRelayedBytes/simTime +
                ...
                "\nhandover delay: " + accumulativeHandoverDelay +
                "\naverage handover delay: " + accumulativeHandoverDelay/count
                ;
