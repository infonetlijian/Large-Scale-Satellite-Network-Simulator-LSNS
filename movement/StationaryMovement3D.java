 ...
 */
public class StationaryMovement3D extends MovementModel {
     ...
     */
    public StationaryMovement3D(Settings s) {
        super(s);
        int coords[];

        coords = s.getCsvInts(LOCATION_S, 3);
        this.loc = new Coord(coords[0],coords[1], coords[2]);
    }
     ...
     */
    public StationaryMovement3D(StationaryMovement3D sm) {
        super(sm);
        this.loc = sm.loc;
    }
    ...
    @Override
    public StationaryMovement3D replicate() {
        return new StationaryMovement3D(this);
    }
